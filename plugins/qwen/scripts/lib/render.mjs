...
function appendActiveJobsTable(lines, jobs) {
  ...
  lines.push(
    "| Job | Kind | Status | Phase | Elapsed | Qwen Session ID | Summary | Actions |",
  );
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
...
export function renderTaskResult(parsedResult, _meta) {
  ...
  const message =
    ...
    "Qwen did not return a final message.";
  return `${message}\n`;
}
...
export function renderStatusReport(report) {
  ...
  if (report.needsReview) {
    ...
    lines.push(
      "Ending the session will trigger a fresh Qwen adversarial review and block if it finds issues.",
    );
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const rawOutput =
    ...
    (typeof storedJob?.result?.qwen?.stdout === "string" &&
      storedJob.result.qwen.stdout) ||
    "";
  if (rawOutput) {
...
export function renderCancelReport(job) {
  const lines = ["# Qwen Cancel", "", `Cancelled ${job.id}.`, ""];
