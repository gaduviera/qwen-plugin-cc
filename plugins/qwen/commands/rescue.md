...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <pro|flash|flash-lite>] [what Qwen should investigate, solve, or continue]"
context: fork
...
- Leave model unset unless the user explicitly asks for one. If they ask for `pro`, map it to `qwen-2.5-pro`. If they ask for `flash`, map it to `qwen-2.5-flash`. If they ask for `flash-lite`, map it to `qwen-2.5-flash-lite`.
