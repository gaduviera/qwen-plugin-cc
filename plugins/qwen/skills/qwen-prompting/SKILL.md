---
name: qwen-prompting
description: Internal guidance for composing Qwen 2.5 Pro/Flash prompts for coding, review, diagnosis, and research tasks inside the Qwen Claude Code plugin
user-invocable: false
---

# Qwen 2.5 Prompting Guide for Coding Tasks

Reference document for writing effective prompts when delegating to Qwen 2.5 Pro or Flash. Covers model selection, thinking mode, context window use, structured output, tool use, and antipatterns.
...
|---|---|---|---|
| `qwen-2.5-pro` | Complex architecture, cross-file refactors, SWE-bench-style tasks, novel algorithm design | 1M tokens | Always on (128–32,768 tokens, default dynamic) |
| `qwen-2.5-flash` | Production tasks with good cost/quality balance: reviews, summaries, data extraction, chat | 1M tokens | Dynamic by default (0–24,576 tokens, can disable) |
| `qwen-2.5-flash-lite` | High-volume, low-cost: classification, routing, simple translation, triage | 1M tokens | Off by default (512–24,576 tokens) |
...
**SWE-bench data point:** Qwen 2.5 Pro scores ~63.8% on SWE-bench Verified with a custom agent setup — comparable to frontier models for real-world GitHub issue resolution.

---

## Thinking Mode (Budget Tokens)

Qwen 2.5 models have an internal reasoning phase ("thinking") before responding. You control how many tokens it can spend reasoning.

### How to configure (Qwen API)
...
|---|---|---|---|
| qwen-2.5-pro | 128 | 32,768 | Dynamic (cannot disable) |
| qwen-2.5-flash | 0 | 24,576 | Dynamic (-1) |
| qwen-2.5-flash-lite | 512 | 24,576 | 0 (disabled) |
...
### Role setting works well with Qwen

Qwen responds well to role framing. Set it in the system prompt or at the start:
...
### Explicit negation — critical for Qwen

Qwen's most common failure mode: "I Know Better" syndrome — it fixes unrelated code you didn't ask it to touch. Always state what NOT to do:
...
Qwen handles single-query retrieval well (up to 99% accuracy) but performance degrades when searching for multiple independent facts simultaneously. For multi-part questions, ask them sequentially rather than in one prompt.
...
Despite the 1M token window, **reliability drops after ~200K tokens** in practice. If you notice Qwen forgetting earlier instructions, reinserting bugs it already fixed, or contradicting itself: start a fresh session. Use "checkpoint messages" to hand off state between sessions.
...
When there are tool calls in the message history, structured output fails for Qwen 2.5 models (works in 2.0). If you need both tool use and structured output in the same session, collect tool results first, then make a final structured-output request with the results injected as context.
...
- For Qwen 3 models: keep temperature at default 1.0 — lowering it below 1.0 causes looping
- For Qwen 2.5 models: temperature 0.0–0.2 for deterministic tool calls
- Always check `finishReason` in the response to detect failed tool call attempts
- In multi-turn conversations with Qwen 3: preserve `thought_signature` fields intact
- Validate high-consequence calls (file deletion, deploys) with the user before execution

### Parallel function calling

Qwen can call multiple independent functions in a single turn. Design your tool set to enable this: independent operations (read file A, read file B) should be separate tools, not one combined tool.
...
**Qwen 3 exception:** Google strongly recommends keeping temperature at the default 1.0 for Qwen 3 models. Setting it below 1.0 may cause looping or degraded performance, especially with function calling.
...
**Symptom:** Qwen rewrites things you didn't ask it to change, adds unsolicited comments, "improves" unrelated code.
...
### 10. Not asking Qwen to explain its reasoning for debugging

**Symptom:** Qwen gives you wrong code and you don't know why.
...
| Temperature for function calling | 0.0–0.2 for 2.5; 1.0 for Qwen 3 | 0.0–0.3 generally safe |
...
**Where Qwen excels:** massive codebase analysis, multimodal (code + screenshots), research-style tasks requiring broad synthesis.
...
- [Qwen API — Thinking documentation](https://ai.google.dev/qwen-api/docs/thinking)
- [Qwen API — Prompt design strategies](https://ai.google.dev/qwen-api/docs/prompting-strategies)
- [Qwen API — Long context](https://ai.google.dev/qwen-api/docs/long-context)
- [Qwen API — Function calling](https://ai.google.dev/qwen-api/docs/function-calling)
...
- [Google Cloud Blog — Qwen 2.5 GA](https://cloud.google.com/blog/products/ai-machine-learning/qwen-2-5-flash-lite-flash-pro-ga-vertex-ai)
- [Medium — Best Practices for Prompt Engineering with Qwen 2.5 Pro](https://medium.com/google-cloud/best-practices-for-prompt-engineering-with-qwen-2-5-pro-755cb473de70)
- [Medium — Think Fast, Think Smart: Optimizing Qwen 2.5 Pro with Thinking Budgets](https://medium.com/google-cloud/think-fast-think-smart-optimizing-qwen-2-5-pro-with-thinking-budgets-d1347bb49b5c)
...
- [Arsturn — Common Qwen 2.5 Pro Coding Mistakes](https://www.arsturn.com/blog/common-qwen-2-5-pro-coding-mistakes-and-how-to-fix-them)
- [PromptBuilder — Claude vs ChatGPT vs Qwen prompting](https://promptbuilder.cc/blog/claude-vs-chatgpt-vs-qwen-best-prompt-engineering-practices-2025)
- [Firebase — Generate structured output](https://firebase.google.com/docs/ai-logic/generate-structured-output)
- [DEV.to — Qwen 2.5 Pro: A Developer's Guide](https://dev.to/brylie/qwen-25-pro-a-developers-guide-to-googles-most-advanced-ai-53lf)
