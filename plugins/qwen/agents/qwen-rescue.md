...
- If the user asks for `pro`, map that to `--model qwen-2.5-pro`.
- If the user asks for `flash`, map that to `--model qwen-2.5-flash`.
- If the user asks for `flash-lite`, map that to `--model qwen-2.5-flash-lite`.
- If the user asks for a concrete model name such as `qwen-2.0-flash-001`, pass it through with `--model`.
